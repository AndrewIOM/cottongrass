...
let aboutYouSection answers translate handler =
    ...
    |> Parser.andQuestion (Parser.choiceQuestion "publishmethod" (translate "Responses from all respondants will be collated and analysed together. As part of this work, we may publish a summary of the responses. We will only publish your responses if you give consent.") [ translate "Do not publish my responses"; translate "Publish anonymised response in full"; translate "Publish response in full with name (and organisation if applicable)" ] (textAnswer 5 answers) true (handler 5)) "always" true
    |> Parser.andQuestion (Parser.choiceQuestion "contactmethod" (translate "If we would like to follow-up your answers, what is the best method of contact?") [ translate "Telephone"; translate "Email"; translate "I don't want to be contacted" ] (textAnswer 6 answers) true (handler 6)) "always" true
    |> Parser.andQuestion (Parser.textQuestion "contactdetail" (translate "Your telephone number or email") "" (textAnswer 7 answers) true (handler 7)) "previous question is not I don't want to be contacted" true
